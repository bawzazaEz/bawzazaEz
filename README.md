...
function RadientPaid:Window(text,maincolor)
   ...
   UserName.Text = "Among nud"
   UserTag.Text = "#" .. " Among nud"
   ...
   return ServerHold
	...
	local serv = win:Server("Among nud Hub", "")

	local Main = serv:Channel("Auto Farm")
	Main:Label("AutoFarm Level")
    
	Main:Toggle("AutoFarm Level",false,function(vu)
	    getgenv().AutoFarm = vu
        do -- Init
        ...
        fastSpawn(function()
                ...
                if getgenv().AutoFarm and game.Players.LocalPlayer.Character:FindFirstChild("Combat") then
                click()
                end
            end)
        end)
        ...
        fastSpawn(function()
            while task.wait() do
                ...
                pcall(function()
                    for i,v in next,game:GetService("Workspace").Npc.Ninja:GetChildren() do
                        if v.Name == "Ninja" and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0,5,5)
                        EquipWeapon(getgenv().Select)
                            game:GetService('VirtualUser'):CaptureController()
                            game:GetService('VirtualUser'):Button1Down(Vector2.new(1280, 672)) 
    ...
    local DropdownSelcat = Main:Dropdown("Select Weapon", weapon, function(kak)
        getgenv().Select = kak
    end)
	Main:Button("Refresh",function()
            DropdownSelcat:Clear()
    weapon = {}
    for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do  
        if v:IsA("Tool") then
            DropdownSelcat:Add(v.Name)
            weapon = {}
    for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do  
    if v:IsA("Tool") then
        table.insert(weapon ,v.Name)
...
spawn(function()
    game.Players.LocalPlayer.Character.HACK.Disabled = true
end)
Main:Slider("WalkSpeed",0,1000,5,function(a)
    game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = a
end)
Main:Slider("JumpPower",0,1000,5,function(a)
